...
namespace UnityEngine.UI
{
    ...
    /// </remarks>
    public class AdvancedGridLayoutGroup : LayoutGroup
    {
        /// <summary>
        ...
        [SerializeField] protected bool m_ApplyToFitConstrainCount = false;
        public bool ApplyFitConstraintCount { get { return m_ApplyToFitConstrainCount; } set { SetProperty(ref m_ApplyToFitConstrainCount, value); } }
        [SerializeField] protected bool m_CenterLastRow = false;
        public bool CenterLastRow { get { return m_CenterLastRow; } set { SetProperty(ref m_CenterLastRow, value); } }
        protected AdvancedGridLayoutGroup()
        { }
        ...
        private void SetCellsAlongAxis(int axis)
        {
            ...
            if (startAxis == Axis.Horizontal)
            {
                ...
                if(ApplyFitConstraintCount && constraintCount > 0)
                {
                    actualCellCountX += (cellCountX - actualCellCountX % cellCountX) % cellCountX;
                }
            }
            else
            {
                ...
                if (ApplyFitConstraintCount && constraintCount > 0)
                {
                    actualCellCountY += (constraintCount - actualCellCountY % constraintCount) % constraintCount;
                }
            }
            ...
            float shiftX = 0;
            if (startAxis == Axis.Horizontal && rectChildren.Count > cellCountX && rectChildren.Count % cellCountX != 0)
            {
                var k = cellCountX - rectChildren.Count % cellCountX;
                shiftX = ((float)k / 2) * cellSize[0] + spacing[0];
            }
            ...
            for (int i = 0; i < rectChildren.Count; i++)
            {
                ...
                var lastRow = positionY == cellCountY - 1;
                if(i < rectChildren.Count)
                {
                    SetChildAlongAxis(rectChildren[i], 0, startOffset.x + (cellSize[0] + spacing[0]) * positionX + (lastRow && CenterLastRow ? shiftX : 0), cellSize[0]);
